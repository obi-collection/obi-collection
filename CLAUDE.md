...
- **統合編集モード（2026-07-04追加）:** URLに `?edit=1` を付けるとFocus調整・Spotify登録・note登録の3モードが同時にONになる。**URLなしでも、ヘッダーの枚数カウンターを素早く3回タップ（スマホ）またはEキー（PC）でON/OFFできる**（`setEditMode()`。そのページ表示内だけ有効で、リロードすると必ずOFFに戻る＝状態は一切保存しない）。下部パネルは統合表示（Focus n · Spotify n · note n）で、Export JSONは `{"focus":{},"spotify":{},"note":{}}` の複合形式 → `python3 merge_all.py edit.json` で一括反映（noteに変更があれば静的ページも再生成）。個別URL（?tune=1/?spotify=1/?note=1）と個別mergeスクリプトも引き続き使える。**モード状態はURLのみで決まり、localStorage等には保存しない**（PC/スマホ間で挙動が食い違わないようにする設計。編集データ自体は各デバイスに溜まり、それぞれExport→mergeでdata.jsに合流させる）
- **オンサイトReview（2026-07-04追加）:** アルバムごとのレビュー本文を `reviews/<slug>.md`（Markdown）で管理し、モーダルの「Review」アコーディオン（開いた時にfetch）と静的ページ（`albums/<slug>.html` に焼き込み・SEO用）の両方に表示。公開済み一覧は `reviews_index.js`（`REVIEWS_INDEX`）が持ち、`merge_review.py` が自動再生成する。登録は `?review=1`（または統合編集モード）でモーダルのテキストエリアに貼り付け→保存で下書き即プレビュー→Export →`merge_review.py`／`merge_all.py`（reviewセクション）で反映。**フック行（歌詞引用）はマージ時に自動除去**（著作権対策）。Markdownレンダラは app.js の `renderMarkdown()` と build_static.py の `render_markdown()` で同一仕様（#〜####見出し・---罫線・-リスト・**太字**・空行区切り段落） URLに `?note=1` を付けるとモーダルにnote記事URLの貼り付け欄が出る。ワークフローは「Ask AIボタンで記事生成 → noteに投稿 → 記事URLを貼って登録」。登録値はlocalStorage（`obi_note`）→「Export JSON」→ `python3 merge_note.py note.json` で `data.js` の `note_url` に反映（`https://note.com/` 始まりのみ受理、空文字でフィールド削除）。merge_note.pyは静的ページも自動再生成し、note記事リンク（`.note-link`）が `albums/<slug>.html` に載る。`note_url` があるアルバムはモーダルに「note」ボタン表示（既存機能）
...
- **データ:** `schedule.js` の `SCHEDULE_DATA.posts[]`（公開・Git管理）。エントリは `{id, month, day, title, text, youtube, image?, category?, enabled}`。年は持たず**毎年繰り返し**（2/29はうるう年のみ発火）。旧「種別（kind）」（release/birthday/other）は2026-07-16に廃止。`category` は動画の内容分類（`mix`/`live`/`interview`＝DJ MIX/LIVE/INTERVIEW、省略可）でエディタの「種別」欄から設定、一覧にタグ表示（2026-07-21追加）
...
- **お披露目投稿（2026-07-16追加、2026-07-17に日付指定対応）:** 一度も投稿されたことのないエントリは「お披露目」として1回投稿され、以降は毎年指定日に投稿される。タイミングはエディタの「お披露目投稿」欄＝`debut` フィールドで決まる: **フィールドなし**＝次の土日（1日1件・ID順＝登録順）／**"YYYY-MM-DD"**＝その日（曜日不問。過ぎた日付は次回実行で追いつく）／**"none"**＝お披露目せず毎年の指定日のみ。お披露目から14日以内に指定日が来る場合、その年の年次分はスキップ（近接二重投稿の防止、`DEBUT_COOLDOWN_DAYS`）。日付指定は週末自動より優先
- **自動反映パイプライン（2026-07-17追加）:** schedule.html の「**保存して反映**」ボタンが下書きJSONを `~/Downloads/obi-schedule-<timestamp>.json` としてダウンロード → launchd常駐の `watch_schedule.py`（`com.obi-collection.watch-schedule.plist`、watch_inbox.pyと同じ構成・**こちらはGit管理対象**）が5秒間隔で検出し、merge_schedule.py → `git add/commit/push` まで自動実行 → ファイルは `~/Downloads/obi-schedule-processed/` へ退避。ログは `~/Library/Logs/obi-watch-schedule.log`。**Export JSON＋手動mergeも従来どおり併用可**（Mac以外の端末はこちら）
