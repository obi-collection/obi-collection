...
function updateAlphaBar() {
    ...
    inner.querySelectorAll('.alpha-btn').forEach(btn => {
        ...
        if (letter === '') {
            ...
            btn.disabled = false;
        } else if (letter === 'compilation' || letter === 'soundtrack' || letter === 'number' || letter === 'r&b' || letter === 'souljazz') {
...
function applyAlphabetFilter() {
    ...
    if (alphabetFilter === 'soundtrack') {
        ...
        if (isDecade) {
            ...
            filteredAlbums = filteredAlbums.filter(a => { const y = a.versions[0].year; return a.genre !== 'r&b' && a.genre !== 'souljazz' && y >= starts[alphabetFilter] && y < ends[alphabetFilter]; });
            ...
            if (alphabetFilter === 'number') {
                filteredAlbums = filteredAlbums.filter(a => a.genre !== 'r&b' && a.genre !== 'souljazz' && /^[0-9]/.test(a._sortName));
            } else {
                filteredAlbums = filteredAlbums.filter(a => a.genre !== 'r&b' && a.genre !== 'souljazz' && a._sortName !== "V.A." && a._sortName !== "O.S.T." && a._sortName.toUpperCase().startsWith(alphabetFilter));
            }
            filteredAlbums.sort(compareArtistThenYear);
        } else {
            applySorting();
